...
export function scoreJob(j, p) {
  // ---- 1. TIMING (default 30%) ------------------------------------------
  // Jobs usually only have a start MONTH (no year), while participants give a
  // full start date. Comparing a hardcoded job year to the participant's year
  // produced a ~365-day gap and a wrong 30%. Compare by closest month instead:
  // try the job month in the participant's year and the adjacent years, and
  // take the smallest gap (handles Dec→Jan wrap-around).
  let timing = 50;
  if (p.startDate) {
    const pStart = new Date(p.startDate);
    let diff;
    if (j.start) {
      diff = Math.abs((new Date(j.start) - pStart) / 86400000);
    } else {
      const m = MONS.indexOf(j.startMonth);
      if (m >= 0) {
        const py = pStart.getFullYear();
        diff = Math.min(
          Math.abs((new Date(py, m, 1) - pStart) / 86400000),
          Math.abs((new Date(py + 1, m, 1) - pStart) / 86400000),
          Math.abs((new Date(py - 1, m, 1) - pStart) / 86400000),
        );
      }
    }
